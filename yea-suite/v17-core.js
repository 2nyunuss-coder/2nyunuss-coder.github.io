...
let histImports=[],histRules={},histPreview=[];
...
async function loadCloud(){
  if(!session?.user?.id)return;try{const [im,ru]=await Promise.all([api('/rest/v1/yea_shift_imports?select=id,file_name,source_type,period_start,period_end,record_count,records,created_at&order=period_start.desc,created_at.desc&limit=500'),api('/rest/v1/yea_shift_unit_rules?select=unit_name,category&order=unit_name.asc&limit=300')]);histImports=im||[];histRules=Object.fromEntries((ru||[]).map(x=>[x.unit_name,x.category]));renderAllHistory()}catch(e){msg('Arşiv verisi alınamadı: '+(e.message||e),'err')}
}
...
async function saveRule(unit,category){histRules[unit]=category;renderReports();try{await api('/rest/v1/yea_shift_unit_rules?on_conflict=user_id,unit_name',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify({user_id:session.user.id,unit_name:unit,category,updated_at:new Date().toISOString()})})}catch(e){msg('Birim ayarı kaydedilemedi: '+(e.message||e),'err')}}
