...
test('versioned entry, offline worker and Android asset whitelist include studio modules',()=>{
  ...
  for(const name of ['app3d-v3.js','studio-models-v3.js','studio-worlds-v3.js','studio-v3.css']){assert.ok(html.includes(name));assert.ok(gradle.includes(name));assert.ok(sw.includes(name));assert.ok(fs.existsSync(path.join(root,'yea-suite/arcade',name)));}
